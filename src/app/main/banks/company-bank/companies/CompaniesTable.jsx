...
function CompaniesTable() {
	...
	// Inject the current categoryId into the list query so the same generic
	// table can serve every dynamically generated COMPANY bank.
	const useListQueryHook = useCallback(
		(args) => useGetCompaniesByCategoryQuery({ ...args, categoryId }),
		[categoryId]
	);
		...
		() => [
			...
			},
			{
				...
				Cell: ({ row }) => (
					<Typography
						component={Link}
						to={`/banks/${categoryId}/company/${row.original.id}`}
						className="underline"
						color="secondary"
						role="button"
					>
		...
		<div className="w-full flex flex-col">
			<FuseScrollbars className="grow overflow-x-auto">
				...
				>
					<GenericCrudTable
						columns={columns}
						useListQueryHook={useListQueryHook}
						saveToStore={false}
