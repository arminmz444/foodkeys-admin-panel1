    ...
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}>
      ...
      >
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mt: 1, maxWidth: 400, mx: 'auto' }}>
          هنوز هیچ آرشیوی برای این سرویس ایجاد نشده است. با کلیک بر روی دکمه "ایجاد آرشیو جدید" اولین آرشیو خود را بسازید.
        </Typography>
      </Paper>
    </motion.div>
  );
...
function ArchivesTab() {
  ...
  const { data: serviceArchives, isLoading: isLoadingArchives, refetch } = useGetArchivesByEntityQuery({
    entityType: 'Service',
    entityId: serviceId,
    page: 0,
    size: 100,
  ...
  const filteredAndSortedArchives = useMemo(() => {
    if (!serviceArchives?.data) return [];

    let result = [...serviceArchives.data];
    ...
    return result;
  }, [serviceArchives, activeTab, archiveTypes, searchText, sortBy]);
  ...
  const handleViewArchive = (archiveId) => {
    ...
    setSelectedArchive(serviceArchives.data.find((a) => a.id === archiveId));
  ...
  const handleCreateArchiveTask = async () => {
    try {
      ...
      try {
        ...
        showSnackbar('آرشیو سرویس با موفقیت ایجاد شد', 'success');
      ...
      <div className="w-full">
            ...
            >
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                  ...
                  <Typography variant="h6" fontWeight={700}>مدیریت آرشیو سرویس</Typography>
                </Box>
        ...
        >
          ...
          >
            ...
            <Typography variant="body2" sx={{ opacity: 0.85 }}>یک نسخه پشتیبان از وضعیت فعلی سرویس ایجاد کنید</Typography>
          </Box>
          <DialogContent>
